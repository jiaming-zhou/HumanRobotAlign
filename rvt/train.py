...
def load_checkpoint(
    ...
    """
    print("Manually loading network weights from {}.".format(path_to_checkpoint))
    ...
    if "r3m" in checkpoint:
        checkpoint["model_state"] = checkpoint["r3m"]
    elif 'model' in checkpoint:
        checkpoint["model_state"]=checkpoint['model']
    ...
    for k, v in pre_train_dict.items():
        if (k in model_dict) or ("model."+k in model_dict):
            if "model."+k in model_dict:
                k="model."+k
    ...
    print("not used: {},{}; not load:{}; model:{}; pretrain:{}".format(
        not_used1,not_used2,
        len(not_load_layers),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log-dir", type=str, default="runs/test")
